<nox-profile-avatar>
  <div class="avatar__upload-container js-avatarUploadContainer">
    <div class="avatar__dz-and-btns js-avatarDzAndBtns">
      <div
        ref="dropZone"
        class="avatar__drop-zone {is--lit:fileOverDropZone} {has--image:chosenImage}"
        style="width:{ imgWidth }px; height:{ imgHeight }px;"
        data-drop-zone-text={ dropZoneText }
        ondragover={ handleDragOver }
        ondragleave={ handleDragLeave }
        ondrop={ handleDrop }
      >
        <img if={ avatar } src={ avatar }>
        <canvas
          ref="avatarCanvas"
          width={ imgWidth }
          height={ imgHeight }
          onmousedown={ handleMouseDown }
        ></canvas>
        <input
          if={ chosenImage }
          ref="scaleSlider"
          type="range"
          orient="vertical"
          min="0"
          max="100"
          value="100"
          onchange={ handleImageScale }
          oninput={ handleImageScale }
        >
      </div>
      <div class="avatar__btn-container">
        <button type="button" class="avatar__choose-btn avatar--is-base-btn js-avatarChooseBtn">{ fileInputText }</button>
        <input
          class="avatar__file-input js-avatarUploadInput"
          type="file"
          accept={ acceptedFormats }
          onchange={ handleFileSelect }
        >
      </div>
    </div>
    <div class="avatar__error-messages avatar--is-hidden js-avatarErrorMessages"></div>
    <div class="avatar__success-message avatar--is-hidden js-avatarSuccessMessage"></div>
  </div>
  
  <style scoped>
    :scope,
    input,
    button, 
    *::after, 
    *::before {
      font: 20px Helvetica, Arial, sans-serif;
    }
    
    :scope * {
      box-sizing: border-box;
    }

    $namespace = '.avatar';
    $animSpeed = 0.25s;

    input[type=range][orient=vertical]{
      width: 8px;
      height: 100%;
      padding: 0;
      margin: 0;
      position: absolute;
      top: 0;
      right: 0;
      z-index: 2;
      writing-mode: bt-lr; /* IE */
      -webkit-appearance: slider-vertical; /* WebKit */
    }

    {$namespace} {

      &__image-container,
      &__upload-container {
        display: inline-block;
        vertical-align: top;
      }

      &__image-container {
        width: 150px;
        margin-right: 5px;
        border: 1px solid gray;
        overflow: hidden;
        position: relative;
      }

      &__upload-container {
        position: relative;
        opacity: 1;
        transition: opacity $animSpeed;
      }

      &__edit-btn {
        width: 100%;
        padding-bottom: 4px;
        border: none;
        border-top: solid 1px #808080;
        border-radius: 0;
        margin-top: -3px;
      }

      &__drop-zone {
        height: 140px;
        margin-bottom: 10px;
        background-image: url('data:image/png;base64,');
        background-size: contain;
        position: relative;
        transition: all $animSpeed;

        img {
          position: absolute;
          top: 0;
          left: 0;
          z-index: 0;
        }

        canvas {
          outline: 3px dashed rgba(0,0,0,0.25);
          outline-offset: -3px;
          z-index: 1;
          position: relative;
        }

        &:after {
          content: attr(data-drop-zone-text);
          color: rgba(0,0,0,0.4);
          font-size: 1.2rem;
          font-weight: bold;
          white-space: nowrap;
          position: absolute;
          left: 50%;
          bottom: 0.5em;
          transform: translateX(-50%);
          background: rgba(255,255,255,0.5);
          padding: 0.1em 0.5em;
          border-radius: 0.5em;
        }

        &.is--lit {

          canvas {
            outline-color: #000;
          }

          &:after {
            color: #000;
          }
        }

        &.has--image {

          canvas {
            cursor: move;
          }

          &:after {
            display: none;
          }
        }
      }

      &__btn-container {
        position: relative;
      }

      &__choose-btn,
      &__upload-btn {
        width: 100%;
        transition: opacity 0.25s;
      }

      &__upload-btn {
        position: absolute;
        top: 0;
        left: 0;
      }

      &__file-input {
        width: 100%;
        height: 100%;
        font-size: 0;
        opacity: 0;
        position: absolute;
        top: 0;
        left: 0;
        cursor: pointer;
      }

      &__upload-progress {
        width: 95%;
        height: 60%;
        border: solid 1px #B0B0B0;
        border-radius: 0.5em;
        overflow: hidden;
        display: block;
        position: absolute;
        bottom: 50%;
        left: 50%;
        transform: translate(-50%, 50%);
        appearance: none;

        &[value] {
          &::-webkit-progress-bar {
            background-color: #eee;
            box-shadow: 0 2px 5px rgba(0, 0, 0, 0.25) inset;
          }

          &::-webkit-progress-value {
            border-radius: 0em 0.4em 0.4em 0em;
            background-image: linear-gradient(#DBFF00, #009913);
            transition: width .25s;
          }
        }
      }

      &__error-messages,
      &__success-message {
        font-weight: bold;
        font-size: 1.25rem;
        text-align: left;
        white-space: pre;
        padding: 0.5em 1em;
        border: solid 2px;
        border-radius: 0.5em;
        margin-top: 10px;
      }

      &__error-messages {
        color: #fff;
        border-color: #843232;
        background: #E85050;
      }

      &__success-message {
        color: #2E5F3F;
        border-color: #2E5F3F;
        background: #52f28b;
      }

      &__api-errors {
        color: #812C3B;
        font-weight: bold;
        padding: 1rem;
        border: dotted 2px #812C3B;
        border-radius: 0.5rem;
        margin: 0.5rem 0;
        overflow: hidden;
        background: #ffc0cb;
        transition: all .25s;

        li {
          list-style-position: inside;
        }
      }

      // == Modifiers ================================================================

      &--is-invisible {
        height: 0;
        min-height: 0;
        opacity: 0;
        padding: 0;
        border-width: 0;
        margin: 0;
        pointer-events: none;
      }

      &--is-hidden {
        display: none;
      }

      &--drop-the-file {
        border-color: #1AC1DF;
      }

      // base styling (like for inputs)
      &--is-base {

        &-btn {
          color: #333;
          font-size: 16px;
          font-weight: 700;
          text-decoration: none;
          padding: 5px 20px;
          border: 1px solid #808080;
          border-radius: 5px;
          cursor: pointer;
          display: inline-block;
          background: #ddd;

          &:hover {
            background-color: #EEEEEE;
            opacity: 1;
          }
        }
      }
    }
  </style>
  
  <script>
    const _self = this;
    const formatPresets = {
      IMAGES: [ 'image/png', 'image/jpeg', 'image/gif' ]
    };
    this.imgScale = 1;
    this.imgOffset = { x: 0, y: 0 };
    this.imgWidth = opts.imgWidth || 300;
    this.imgHeight = opts.imgHeight || 300;
    this.dropZoneText = opts.dropZoneText || 'Drop Image Here';
    this.fileInputText = opts.fileInputText || 'Choose Image';
    this.acceptedFormats = opts.acceptedFormats || formatPresets.IMAGES.join(',');
    this.fileOverDropZone = false;
    this.chosenImage = undefined;
    this.prevX = 0;
    this.prevY = 0;
    this.onRender = opts.onRender || undefined;
    this.avatar = opts.avatar || undefined;

    this.reset = function(){
      _self.chosenImage = undefined;
      _self.ctx.clearRect(0, 0, _self.imgWidth, _self.imgHeight);
      _self.update();
    };

    this.handleDragOver = function(ev){
      ev.preventDefault();

      _self.fileOverDropZone = true;
      _self.update();
    };

    this.handleDragLeave = function(ev){
      if( ev ) ev.preventDefault();

      _self.fileOverDropZone = false;
      _self.update();
    };

    this.handleDrop = function(ev){
      ev.preventDefault();

      // TODO - check `formatPresets`

      if( _self.refs.scaleSlider ) _self.refs.scaleSlider.value = 100;
      _self.handleDragLeave();
      handleFile(ev.dataTransfer.files);
    };

    this.handleFileSelect = function(ev){
      handleFile(ev.target.files);
    };

    this.handleMouseMove = function(ev){
      var offsetX = ev.offsetX - _self.startPos.x;
      var offsetY = ev.offsetY - _self.startPos.y;
      var delta = 2;

      if( Math.abs(offsetX) > delta || Math.abs(offsetY) > delta ){
        var xDiff = offsetX - _self.prevX;
        var yDiff = offsetY - _self.prevY;
        _self.imgOffset.x += xDiff;
        _self.imgOffset.y += yDiff;

        renderImage();
      }

      _self.prevX = offsetX;
      _self.prevY = offsetY;
    };

    this.handleMouseUp = function(ev){
      _self.refs.avatarCanvas.removeEventListener('mousemove', _self.handleMouseMove);
      _self.refs.avatarCanvas.removeEventListener('mouseup', _self.handleMouseUp);
    };

    this.handleMouseDown = function(ev){
      _self.startPos = { x: ev.offsetX, y: ev.offsetY };
      _self.refs.avatarCanvas.addEventListener('mousemove', _self.handleMouseMove);
      _self.refs.avatarCanvas.addEventListener('mouseup', _self.handleMouseUp);
    };

    this.handleImageScale = function(ev){
      _self.imgScale = ev.target.value / 100;
      renderImage();
    };

    function renderImage(){
      var img = _self.chosenImage;
      var renderedImage;

      _self.ctx.clearRect(0, 0, _self.imgWidth, _self.imgHeight);
      _self.ctx.drawImage(img, _self.imgOffset.x, _self.imgOffset.y, img.width * _self.imgScale, img.height * _self.imgScale);
      renderedImage = _self.ctx.canvas.toDataURL('image/jpeg', 0.8);

      if( _self.onRender ) _self.onRender(renderedImage);
    }

    function handleFile(files){
      var file = files[0];
      var reader = new FileReader();

      reader.addEventListener('load', function(){
        _self.chosenImage = new Image();
        _self.chosenImage.onload = function(){
          renderImage();
          _self.update();
        };
        _self.chosenImage.src = reader.result;
      }, false);

      // Read in the image file as a data URL.
      reader.readAsDataURL(file);
    }

    function handleMount(ev){
      _self.ctx = _self.refs.avatarCanvas.getContext('2d');
    }

    this.on('mount', handleMount);
  </script>
</nox-profile-avatar>